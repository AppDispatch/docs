...
|---|---|---|
| **Scope** | One flag | Reusable across flags |
...
When adding a targeting rule to a flag, select **Segment** as the rule type and pick from the dropdown. The rule card shows:
...
The segment detail view shows a **Referenced by** list — every flag that uses this segment as a targeting rule. You can't delete a segment that's still referenced by flags. Remove the segment rules from those flags first, then delete.
