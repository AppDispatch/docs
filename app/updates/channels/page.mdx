...
Channels and branches control which updates reach which devices.

## How they relate

**Branch** — A named stream of updates. Each update is published to a branch.

**Channel** — What devices connect to. Each channel points to a branch. When a device requests the latest update for a channel, the server looks up the branch and returns the latest update on that branch.

```mermaid
flowchart LR
  P["Channel: production"] --> M["Branch: main"] --> U1["Update v2.1"]
  S["Channel: staging"] --> ST["Branch: staging"] --> U2["Update v2.2-rc1"]
  B["Channel: beta"] --> M
```

Multiple channels can point to the same branch. This lets you "promote" an update by repointing a channel rather than re-publishing.
...
Override specific users to receive updates from a different branch, regardless of their channel configuration:
...
Channels can enforce a **minimum runtime version**. If a device's runtime version is below the minimum, the server tells the device to update via the app store instead of delivering an OTA update.
