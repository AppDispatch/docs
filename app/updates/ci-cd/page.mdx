...
Automate OTA deployments with GitHub Actions so every push to `main` publishes an update — unless native code changed.
...
2. **Publishes an OTA update** — If only JavaScript changed, it installs the Dispatch CLI and publishes the update to your chosen channel.
    ...
    inputs:
      channel:
        ...
        options:
          ...
          - canary
      is_critical:
        description: 'Mark as critical update (force immediate reload)'
        required: false
        default: false
        type: boolean
      rollout_percentage:
  ...
  warn-native-change:
    ...
    steps:
      - name: Skip OTA - native changes require full build
        run: |
          echo "::warning::Native dependencies changed — OTA update skipped."
          echo "Changes to iOS/Android native code, Podfile, build.gradle, or package dependencies"
          echo "require a full app store build."
          echo ""
          echo "After submitting the new binary, future JS-only pushes will resume OTA deploys."
```
...
If native changes are detected, the OTA deploy is skipped and a warning annotation appears on the workflow run.
...
- **Mark as critical** — Forces an immediate reload on devices instead of waiting for the next app launch
